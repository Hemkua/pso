...
from utils import data_handle_v1, data_handle_v2,load_data
from config.config import args, kernel, data_src, data_path

def fitness_function(position,data):
    X_train, X_test, y_train, y_test = data
    svclassifier = SVC(kernel=kernel, gamma = position[0], C = position[1],decision_function_shape='ovo' )
...
def pso_svm(data):
    ...
    while iteration < args.n_iterations:
        ...
        for i in range(args.n_particles):
            ...
            print(type(fitness_cadidate),len(fitness_cadidate))
...
'''
def main():
    X_train, X_test, y_train, y_test = load_data('提取有用的特征.csv', test_size=0.2, random_state=42)
    data = [X_train, X_test, y_train, y_test]
    pso_svm(data)
